...
export async function generatePreset(
  ...
  opts: {
    ...
    /**
     * Emit one model instead of the whole library, loaded at startup, to a
     * preset of its own. This is the calibration probe: it has to run the
     * settings the router would run, so it is generated by this function
     * rather than by a second copy of the same emission rules.
     */
    only?: { modelId: string; fileName: string }
  } = {}
  ...
  const selected = only
    ? modelEntries.filter((entry) => entry.modelId === only.modelId)
    : modelEntries
  ...
  lines.push('[*]')
  // Kuru Fit (the default) computes ctx-size and n-gpu-layers from GGUF
  // metadata, so llama.cpp's own fit — which would override both while on —
  // is forced off and is not a user setting. With Kuru Fit off (legacy Jan
  // fitting), the user's own fit / fit-target / fit-ctx values apply.
  if (config.kuru_fit === false) {
    // fit default = 'on'
    if (config.fit === false) {
      lines.push('fit = off')
    }
  ...
  for (const { modelId, configPath } of selected) {
    let mc: ModelYaml
    try {
    ...
    // The context planner budgets a specific set of GPUs. Without naming them
    // llama.cpp spreads the model over every device it can see, which spends
    // memory the plan never counted.
    if (typeof mc.device === 'string' && mc.device.trim().length > 0) {
      lines.push(`device = ${escapeIniValue(mc.device.trim())}`)
    }
    if (typeof mc.tensor_split === 'string' && mc.tensor_split.trim().length > 0) {
      lines.push(`tensor-split = ${escapeIniValue(mc.tensor_split.trim())}`)
    }
    // Emitted even when it matches llama.cpp's own default, unlike the `[*]`
    // block above: the global section can carry `split-mode = none`, which
    // registers only the main device and aborts the load of any plan that
    // names a second card. The per-model value has to be able to say `layer`.
    if (typeof mc.split_mode === 'string' && mc.split_mode.trim().length > 0) {
      lines.push(`split-mode = ${escapeIniValue(mc.split_mode.trim())}`)
    }
    ...
    // The probe sends no request, and the router loads lazily, so without
    // this it would measure nothing.
    lines.push(only ? 'load-on-startup = true' : 'load-on-startup = false')
    lines.push('')
  }

  const fileName = only ? only.fileName : 'router.preset.ini'
  const outPath = await joinPath([providerPath, fileName])
  const tmpPath = await joinPath([providerPath, `${fileName}.tmp`])
